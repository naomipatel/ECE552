...
//   //victim cache
//   if ((addr + nbytes) > ((addr & ~vcp->blk_mask) + vcp->bsize))
//     fatal("cache: access error: access spans block, addr 0x%08x", addr);
...
//   /* cache block not found in L1 cache */
//   /* check victim cache */
//   if (CACHE_TAGSET(vcp, addr) == vcp->last_tagset)
//     {
//       /* hit in the same block */
//       blk = vcp->last_blk;
//       goto cache_fast_hit;
...
//     {
//       /* higly-associativity cache, access through the per-set hash tables */
//       int hindex = CACHE_HASH(vcp, tag);

//       for (blk=vcp->sets[set].hash[hindex];
// 	   blk;
// 	   blk=blk->hash_next)
// 	{
// 	  if (blk->tag == tag && (blk->status & CACHE_BLK_VALID))
...
//   // check Victim cache
//   // - if found, normal goto cache_hit () and write into L1 (add latency for writing into L1)
//   // - if miss in now both, normal cache miss and write into L1
//   // - 
//   /* miss in both L1 and Victim Cache*/
//   /* **MISS** */
//   cp->misses++;
//   vcp->misses++;
